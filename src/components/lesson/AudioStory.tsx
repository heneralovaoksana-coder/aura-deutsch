...
export default function AudioStory({ story, onComplete }: AudioStoryProps) {
  ...
  return (
    <div className="flex flex-col items-center justify-center p-6 h-full">
      ...
      >
         <div className="w-20 h-20 mx-auto mb-6 bg-gradient-to-br from-primary to-pink-neon rounded-full p-[2px] shadow-[0_0_20px_rgba(255,0,122,0.4)]">
           <div className="w-full h-full bg-bg-deep rounded-full flex items-center justify-center relative border border-black overflow-hidden">
             {isPlaying && (
                <motion.div 
                  className="absolute inset-0 bg-pink-neon/20"
                  animate={{ scale: [1, 1.2, 1], opacity: [0.5, 0, 0.5] }}
                  transition={{ repeat: Infinity, duration: 1.5 }}
                />
             )}
             <button onClick={handlePlay} className="text-white hover:text-pink-neon transition-colors relative z-10">
               ...
               {activeQuestion.options.map((opt, idx) => (
                 <button
                   ...
                   className={`w-full p-4 rounded-xl border text-left transition-colors font-medium text-sm ${
                     selectedOpt === idx
                       ? "border-pink-neon bg-pink-neon/20 text-white shadow-[0_0_15px_rgba(255,0,122,0.2)]"
